...
def employees_by_region(data):
    pivot_table = pd.pivot(data, index = "sales_region", values="employee_id", aggfunc=pd.Series.nunique)
...
def exit_program(data):
    sys.exit(0)
#display the top level menu of the user options. 
def display_menu(data):
    menu_options = (
        ("Show the first n rows of data", display_rows),
        ("Show the number of employees by reigon", employees_by_region), 
        ("Exit the program", exit_program)
    )
    print("\nPlease choose from among these options:")
    for index, (discription,_) in enumerate(menu_options):
        print(f"{index+1}: {discription}")

    num_choices = len(menu_options)
    choice = int(input(f"Please Select an option between 1 and {num_choices}: "))
